	...
	methods: {
		...
		async authenticate(ctx, route, req) {
			...
			if (auth && auth.startsWith("Basic")) {
				...
				const credentials = new Buffer(token, 'base64').toString('utf-8').split(':')
				if(credentials[1] === '123456') {
					// Returns the resolved user. It will be set to the `ctx.meta.user`
					const user = await ctx.broker.call('user.getById', credentials[0])
					if(!user){
